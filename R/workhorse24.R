DownloadAndBuild24Hours <- function(config) {
  ...
  .message(paste0("Downloading data for ", "rolling table", "..."), config = config)
  ...
  cc <- foreach(i = 1:length(map)) %dopar% {
    ...
    if(length(map[[i]]) > 1){
      ...
      .Download24HourDataFromNWIS(sites = map[[i]],
  ...
  .message(paste0("Building NetCDF Files for ", "rolling table", "..."), config = config)
  ...
  queue <- BuildSubsetQueue(suffix = "rolling", config = config, conn = conn)
  BuildNetCDF(data = data, queue = queue, cluster = cluster, suffix = "rolling", config = config, conn = conn)
